...
import MarkdownRenderer from "../common/MarksDownRender";
...
const ChatInterface = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const errorMessage = {
        role: "assistant",
        content: "Sorry, I encountered a error, Please try again",
        timestamp: Date.now(),
      };
  ...
  return (
    <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden ">
      {/* Messages Area */}
      <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 via-white/50 to-steal-50/50">
        {history.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="w-16 h-16 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 flex items-center justify-center mb-4 shadow shadow-emerald-500/10">
              <MessageSquare
